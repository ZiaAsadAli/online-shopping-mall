...
require_once("perpage.php");
		...
		foreach($_POST["search"] as $k=>$v){
			if(!empty($v)) {
				...
				switch($k) {
					case "name":
						$sname = $v;
						$queryCondition .= "status='unautherized' AND code LIKE '%" . $v . "%'";
						break;
	...
	$sql = "SELECT * FROM products_list   " . $queryCondition ."";
	$sql1 = "SELECT * FROM products_list  where status='unautherized' " ;
	$href = 'new_products.php';					
...
<div class="col-md-9">
	<div class="mens-toolbar">
          <div class="sort">
			...
			<form name="frmSearch" method="post" action="new_products.php">
			...
			<p>
				...
				<?php
					...
					?>
          <tr>
					<td><a href="asingle.php?UserId=<?php echo $result[$k]["product_id"]; ?>"><?php echo $result[$k]["code"]; ?></a></td>
					<td><?php echo $result[$k]["product_name"]; ?></td>
					<td><?php echo $result[$k]["product_price"]; ?></td>
					<td><?php echo $result[$k]["category"]; ?></td> 
					<td><?php echo $result[$k]["brands"]; ?></td> 
					<td><?php echo $result[$k]["status"]; ?></td> 
					<td>
					 <a class="btnDeleteAction" onclick="return confirm('Are You Sure To Authorized The Product')" href="authorize.php?id=<?php echo $result[$k]["product_id"]; ?>">Authorize</a>
					</td>
					</tr>
					<?php
						}
