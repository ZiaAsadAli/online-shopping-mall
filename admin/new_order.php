...
require_once("perpage.php");
		...
		foreach($_POST["search"] as $k=>$v){
			if(!empty($v)) {
				...
				switch($k) {
					case "name":
						$sname = $v;
						$queryCondition .= "status='In Process' and order_no LIKE '%" . $v . "%'";
						break;
					}
			}
		}
	...
	$sql1 = "SELECT * FROM cart where status='In Process' " ;
	$href = 'new_order.php';					
	...
	<div class="mens-toolbar">
          <div class="sort">
			...
			<p>
				<tr>
				<th colspan=6>
				<input type="text" placeholder="Enter Order No" name="search[name]" class="demoInputBox" value="<?php echo $sname; ?>"/>
				...
				<?php
					...
					?>
          <tr>
					<td><a title="Click To View Detail" href="order_details.php?ordercode=<?php echo $result[$k]["order_no"]; ?>"><?php echo $result[$k]["order_no"]; ?></td>
					...
					<td>
					 <a title="Click To View Detail" class="btnDeleteAction" onclick="return confirm('Did You Confrim the by buyer and check buyer profile')" href="orderpprocess.php?orid=<?php echo $result[$k]["id"]; ?>">Process</a>
					</td>
