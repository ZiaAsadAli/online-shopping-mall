...
{
	$product_id   = $_POST["product_id"]; //product code
	$product_qty    = $_POST["quantity"]); //product quantity
	$product 		= array();
	$found 			= false;
	//fetch item from Database using product code
	$statement = $mysqli_conn->prepare("SELECT product_name, product_price FROM products_list WHERE product_id=? LIMIT 1");
	...
	while($statement->fetch()){
		...
		$new_product = array( array('name'=> $product_name, 'price'=> $product_price, 'p-id'=>$product_id, 'qty'=>$product_qty)); //prepare new product
		if(isset($_SESSION["products"]))
        {
            foreach ($_SESSION["products"] as $cart_itm)  //loop though items
            {
				if($cart_itm["p-id"] == $product_id){ //if item found in the list, update with new Quantity
					$product[] = array('name'=>$cart_itm["name"], 'p-id'=>$cart_itm["p-id"], 'qty'=>$product_qty, 'price'=> $cart_itm["price"]);
                    $found = true;
                }else{
                   	//else continue with other items
				    $product[] = array('name'=>$cart_itm["name"], 'p-id'=>$cart_itm["p-id"], 'qty'=>$cart_itm["qty"], 'price'=> $cart_itm["price"]);
                }
            }
