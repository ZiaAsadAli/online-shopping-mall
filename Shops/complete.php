...
$date1=date('M-d-Y');
$adname=$_SESSION['shpname'];
$ademil=$_SESSION['shpemail'];
$adno=$_SESSION['shpno'];
$a=ordercode();
...
foreach ($_SESSION["cart_item"] as $addi){
	...
	$mysqli_conn->query("INSERT INTO `cart`(`product_name`, `product_qty`, `product_price`, `product_code`, 
									...
									'".$adname."','".$ademil."','".$date1."','In Process','".$a."','".$addi["brand"]."','$adno')");
									
									
									
										}
	unset($_SESSION["cart_item"]);
	echo '<script type="text/javascript">alert("Product Added Successfully");window.location=\'index.php\';</script>';
?>